...
- 已修复底部“1至74点一一对应”这类说明行误参与转换的问题：当行内任意单元格包含“一一对应”时，转换阶段会直接跳过，且 `pin_parser.py` 会返回空针脚，避免误生成 `Net75` 之类的无效记录。
- 已补充“一分三/一分多”说明行过滤：像 `PW01-X43的44点一分三`、`PW01-X43 的 44 点一分三` 这类分支说明会在 `cable_converter.py` 中按整行识别并跳过，同时 `pin_parser.py` 与 `connector_normalizer.py` 增加保守拦截，避免说明文字误入针脚或连接器字段。
- 已新增普通接线中多对一/数量不一致行过滤规则：当起点针脚与终点针脚数量不一致时，整行直接跳过，不再按较短数量生成。
- 上述数量不一致跳过规则同时作用于上传预览和最终导出，确保 `build_preview_rows(...)` 与 `build_all_rows(...)` 转换结果一致。
- 已调整工作簿 Sheet 遍历规则：上传解析阶段会按工作簿顺序跳过第一个说明 Sheet，因此它不会进入字段识别、预览结果、`parsed_sheets` 存储和最终导出。
- 已完成前端轻量美化：统一 `App.vue` 页面壳层、浅灰背景、白色卡片与更适合表格阅读的宽度。
- 已优化上传页与结果页展示层次，保留原有上传、转换、下载接口逻辑不变。
- 已为 Preview 页面增加“精简 / 详细”切换，默认进入精简模式，仅展示文件名、有效 Sheet 数量、警告数量、当前 Sheet 切换和转换结果预览；详细模式保留字段识别、原始预览、warnings/errors 等核对信息。
